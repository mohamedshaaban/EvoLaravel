...
function SeatchartJS(seatMap, seatTypes) { // eslint-disable-line no-unused-vars
    ...
    //else if (seatMap.rows > 50 || seatMap.cols > 50) {
    //throw new Error("Invalid parameter 'seatMap' supplied to SeatchartJS. " +
    ...
    //}
        ...
        alert("'row' and 'cols' properties cannot be integers smaller than 2.");
    }
    ...
    this.createMap = function createMap(containerId) {
        ...
        for (var i = 0; i < seatMap.rows; i += 1) {
            ...
            for (var j = 0; j < seatMap.cols; j += 1) {
                row.appendChild(createSeat('available', i  , i + '_' + j));//rowIndex + (j + 1)
            }
        ...
        //console.log(parseInt(seatMap.cols) + 1);
        seatMapContainer.style.width = ((width + margins) * (parseInt(seatMap.cols) + 1)) + margins + 100 +'px'; //((width + margins) * (seatMap.cols + 1)) + margins//'{0}px'.format(((width + margins) * (parseInt(seatMap.cols) + 1 )) + margins);
        //console.log(seatMapContainer.style.width);
        var front = seatMapContainer.getElementsByClassName('seatChart-front')[0];
        front.style.width = '{0}px'.format(((width + margins) * parseInt(seatMap.cols)) - margins);
